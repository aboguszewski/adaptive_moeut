...
class SigmaMoE(torch.nn.Module):
    ...
    def get_reg_loss(self) -> torch.Tensor:
        ...
        # FIX: cat on dim=0 perfectly merges all active tokens from all loops
        # into a single [Total_Active_Tokens, Experts] tensor. No flatten needed!
        all_sels = torch.cat(self.sel_hist, dim=0)
        loss = entropy_reg(all_sels, -1) # Usually expert dim is the last one (-1) for a 2D tensor
